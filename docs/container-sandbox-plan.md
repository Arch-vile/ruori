...
- **`ruori` does not know or care what agent (if any) runs.** The container is entirely the user's own devcontainer setup (their own Dockerfile). `ruori`'s job in container mode is only to start a *bare* tmux session inside the container and attach iTerm2 to it — no command is ever injected. Whatever starts (an agent, a shell, nothing) is fully up to the container image (its own `CMD`/entrypoint, a tmux `default-command`, or the user typing it by hand). This makes the feature agent-agnostic by omission rather than by configuration — no `agent-command`-style directive needed at all.
