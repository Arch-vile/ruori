...
Eight directives go in the same `.ruori.conf` from step 2:
...
- **`container-shared-volume <absolute-container-path>`** — repeatable.
  ...
  mkdir -p /pnpm-store && chown <container-user> /pnpm-store` and two
  lines, `ENV npm_config_store_dir=/pnpm-store` and `ENV
  pnpm_config_store_dir=/pnpm-store` (both: pnpm ≤10 reads only the first, pnpm 11+ only the second; an
...
- **`container-init <cmd>`** — repeatable. An arbitrary shell command
  ...
  generic rather than a directive per one-time setup need — propose it
  for e.g. pointing the container's own git at a tracked,
  container-only ignore file (`container-init git config --global
  core.excludesFile "$PWD/.devcontainer/container.gitignore"`, with
  that file tracked in the repo — no `container-copy` needed, it's
  already visible via the worktree bind mount) whenever an artifact is
  generated by container-side tooling only (see the `container-volume`
  bullet above) and the goal is just to stop `git status` inside the
  container from showing it — not something every collaborator or the
  host needs to see. `$PWD` inside the command resolves to the worktree
  root.
...
build` with the **main worktree root** as the build context (`docker
build -f <container-file> <main-worktree-root>`), regardless of where
`container-file` points. So if the Dockerfile lives at
`.devcontainer/Dockerfile` (the default) and these two files sit next
to it in `.devcontainer/`, every `COPY` of them — and of anything else
you add later — must be written as `COPY .devcontainer/<file> ...`,
never a bare `COPY <file> ...`. A bare path will fail the build with a
"not found" checksum error the first time someone actually builds the
image, which this guide never does (see "Don't run, build, or test
anything" below) — so double-check this yourself before presenting the
plan; don't rely on a build to catch it.
